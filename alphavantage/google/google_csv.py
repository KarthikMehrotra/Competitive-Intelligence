...
url = 'https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=GOOG&apikey=9GEPM841QDY4HGPG'
r = requests.get(url)
data = r.json()
...
csv_file_name = 'extracted_data_gcp_google_v2.csv'

csv_file_path = os.path.join(folder_path, csv_file_name)

with open(csv_file_path, 'w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['url', 'content', 'entity', 'salience', 'verb', 'timestamp', 'company_name'])
    ...
    for item in extracted_data:
        ...
        for entity in entities:
            ...
            salience = entity['salience']
            writer.writerow([url, content, name, salience, '', item['timestamp'], 'google'])

        verbs = item['verbs']
        for verb in verbs:
            writer.writerow([url, content, '', '', verb, item['timestamp'], 'google'])
